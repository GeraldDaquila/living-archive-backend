...
def test_v385_identity_and_core_integrity():
    assert 'APP_VERSION = "v385"' in MAIN
    assert 'DEPLOYMENT_FINGERPRINT = "USE-v385-open-transition-doorway-fit"' in MAIN
...
def test_v385_open_transition_preserves_direct_visitor_authority():
    tree = ast.parse(MAIN)
    fn = next(n for n in tree.body if isinstance(n, ast.FunctionDef) and n.name == "_v385_finalize")
