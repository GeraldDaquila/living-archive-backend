...
# Parse before any semantic assertions so syntax failures are caught first.
...
# The grief refinement must remain local: no replacement of the transition machinery.
assert "def _v390_finalize" in main_source
assert "use_core.generate_llm_response = _v390_finalize" in main_source
assert "expected protected core blob sha={EXPECTED_CORE_BLOB_SHA}" in main_source
...
# Confirm the validator itself remains executable.
compile(tree, "main.py", "exec")
compile(core_tree, "use_core.py", "exec")

# Confirm the protected core blob identity directly from the local file bytes.
