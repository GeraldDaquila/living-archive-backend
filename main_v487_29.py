...
if not _CORE_PATH.exists(): raise RuntimeError("USE v487.29 package integrity failure: use_core.py is missing.")
_core_bytes=_CORE_PATH.read_bytes(); _core_runtime_sha=hashlib.sha1(f"blob {len(_core_bytes)}\0".encode()+_core_bytes).hexdigest()
if _core_runtime_sha!=EXPECTED_CORE_BLOB_SHA: raise RuntimeError("USE v487.29 package integrity failure: protected core mismatch.")
...
if _original_handle_query is None: raise RuntimeError("USE v487.29 package integrity failure: API query handler unavailable.")
if not callable(_original_evidence_sufficiency_unavailable_response): raise RuntimeError("USE v487.29 package integrity failure: evidence-gap boundary unavailable.")
def _sanitize_visitor_output(text): return re.sub(r"\bUSE\b","The Guide",str(text or "")).replace("..",".")
def _normalize_query(text): return re.sub(r"\s+"," ",str(text or "").strip().casefold().replace("’","'").replace("‘","'").replace("`","'").replace("–","-").replace("—","-"))
...
def _weighted_inquiry_profile(query):
    q=_normalize_query(query); p={"risk":0.0,"recommendation":0.0,"navigation":0.0,"foundation":0.0,"lived":0.0,"conceptual":0.0,"grief":0.0,"specialized":0.0,"ai_truth":0.0}
    ...
    if _has_archive_help_request(q): p["recommendation"]+=0.82
    ...
    if _has_experiential_stance(q) or _has_experiential_state(q): p["lived"]=0.86
...
def _candidate_sentences(query,docs):
    q=_normalize_query(query); terms=set(re.findall(r"[a-z]{4,}",q)); candidates=[]; seen=set()
    for doc in docs:
...
def _canonical_family_primary(profile): return (_canonical_catalog(profile).get("direct") or None)
...
def _canonical_role(title,text,profile):
    ...
    if profile.get("ai_truth"):
        ...
        if "knowledge stewardship" in low or "information to wisdom" in low or "meaning crisis" in low: return "ai_truth_adjacent"
...
def _recommendation_anchor_score(query,claim,profile):
    ...
    if profile.get("ai_truth"): score += {"ai_truth_direct":80,"ai_truth_adjacent":30}.get(role,0)
...
def _recommendation_rationale(primary,profile):
    if profile.get("grief") and "continuity" in str(primary.get("title","")).casefold(): return "I’m recommending this first because it approaches grief and the human search for continuity directly, making it a more immediate place to reflect on the experience of losing someone you love."
    if profile.get("grief"): return "I’m recommending this first because it speaks directly to grief and loss, rather than asking you to begin with a more specialized interpretation of what happens after death."
...
def _recommendation_answer(query,docs,profile):
    ...
    return f"A useful place to begin with this question is {primary_link}.\n\n{_recommendation_wisdom(profile)}\n\n{_recommendation_foothold(profile)}\n\n{_recommendation_rationale(primary,profile)}\n\nThis doorway is offered as a reflection gateway, not as a complete explanation or prescription. It is one place to begin noticing what this question opens for you.\n\nYou can stay with this lens, follow the connected material, or return with another part of the question that feels more relevant."
def _build_lived_experience_answer(query,docs,profile):
    claims=_extract_claims(_candidate_sentences(query,docs))
    if not claims: return "Uncertainty, loneliness, fear, grief, and other difficult experiences can be part of ordinary human life. You do not have to resolve the experience before you can begin listening to the question it raises."
...
def _build_factual_answer(query,docs):
    ...
    primary=claims[0]; parts=[f"The closest supported material I found is [{primary['title']}]({primary['url']}).",f"Taken together, the available material approaches {subject} through several related perspectives rather than a single fixed definition."]
...
def _calibration_contract_audit(answer,mode,risk=False):
    ...
    if mode=="foundation": return {"human_reality":"human questions" in low,"humane_foothold":bool(re.search(r"begin|finding your bearings|enter with a question|follow a pathway|move outward",low)),"epistemic_boundary":True,"risk_routing":True,"outward_gateway":True,"teacherly_sovereignty_voice":bool(re.search(r"finding your bearings|enter with a question|move outward",low))}
    return {"human_reality":bool(re.search(r"grief|grieving|loss|loneliness|emptiness|experience|question|uncertain|discernment|truth|human|claim",low)),"humane_foothold":bool(re.search(r"for today|for now|next step|humane next step|notice|stay with|place to begin|ask|verify|smallest question",low)),"epistemic_boundary":bool(re.search(r"interpretive|possibilit(?:y|ies)|established fact|established knowledge|personal meaning|reflection gateway|not a complete explanation|not the whole answer|what can be known|what is interpretation|verify|not a claim",low)),"risk_routing":True,"outward_gateway":gateway,"teacherly_sovereignty_voice":bool(re.search(r"wisdom|you can|for today|for now|stay with|what kind of claim|finding your bearings|one way into the question|rather than",low))}
def _catalog_audit(catalog,needle):
    item=catalog.get("direct") or {}; return item.get("title","")==needle and item.get("url","").startswith("https://")
_V487_FOUNDATION_AUDIT=_foundation_teacherly_answer("What is the Living Archive?")
_V487_AI_QUERY="I keep wondering whether AI is making it harder to know what is actually true. Where should I begin in the Living Archive?"; _V487_AI_PROFILE=_inquiry_profile(_V487_AI_QUERY); _V487_AI_TRUTH_CATALOG=_canonical_catalog(_V487_AI_PROFILE); _V487_AI_TRUTH_AUDIT=_recommendation_answer(_V487_AI_QUERY,[],_V487_AI_PROFILE)
_V487_GRIEF_QUERY="I’m struggling with grief after losing someone I love, and I keep wondering whether I should let go or hold on. Where should I begin in the Living Archive?"; _V487_GRIEF_PROFILE=_inquiry_profile(_V487_GRIEF_QUERY); _V487_GRIEF_CATALOG=_canonical_catalog(_V487_GRIEF_PROFILE); _V487_GRIEF_AUDIT=_recommendation_answer(_V487_GRIEF_QUERY,[],_V487_GRIEF_PROFILE)
_V487_LONELINESS_QUERY="I’m struggling with loneliness. Is there anything in the Living Archive that might help me think about it?"; _V487_LONELINESS_PROFILE=_inquiry_profile(_V487_LONELINESS_QUERY)
_V487_LONELINESS_AUDIT=_recommendation_answer(_V487_LONELINESS_QUERY,[{"title":"The Silent Epidemic: Exploring Loneliness, Despair, Emptiness, and the Redemptive Power of the Eternal Now","url":"https://geralddaquila.com/example-loneliness","text":"Loneliness and emptiness can be approached as questions of human experience."}],_V487_LONELINESS_PROFILE)
_V487_FOUNDATION_CONTRACT_AUDIT=_calibration_contract_audit(_V487_FOUNDATION_AUDIT,"foundation"); _V487_AI_CONTRACT_AUDIT=_calibration_contract_audit(_V487_AI_TRUTH_AUDIT,"recommendation"); _V487_GRIEF_CONTRACT_AUDIT=_calibration_contract_audit(_V487_GRIEF_AUDIT,"recommendation"); _V487_LONELINESS_CONTRACT_AUDIT=_calibration_contract_audit(_V487_LONELINESS_AUDIT,"recommendation")
if "Living Archive" not in _V487_FOUNDATION_AUDIT: raise RuntimeError("USE v487.29 invariant audit failed: foundation.")
if not _catalog_audit(_V487_AI_TRUTH_CATALOG,"Truth in the Age of AI: Why Discernment Is Becoming a Survival Skill"): raise RuntimeError("USE v487.29 invariant audit failed: AI truth catalog.")
if not _catalog_audit(_V487_GRIEF_CATALOG,"Death, Grief, and the Human Search for Continuity"): raise RuntimeError("USE v487.29 invariant audit failed: grief catalog.")
if _V487_AI_PROFILE["action"]!="recommendation": raise RuntimeError(f"USE v487.29 invariant audit failed: AI movement task={_V487_AI_PROFILE['action']}")
if _V487_GRIEF_PROFILE["action"]!="recommendation": raise RuntimeError(f"USE v487.29 invariant audit failed: grief movement task={_V487_GRIEF_PROFILE['action']}")
if _V487_LONELINESS_PROFILE["action"]!="recommendation": raise RuntimeError(f"USE v487.29 invariant audit failed: loneliness movement task={_V487_LONELINESS_PROFILE['action']}")
if "Truth in the Age of AI" not in _V487_AI_TRUTH_AUDIT: raise RuntimeError(f"USE v487.29 invariant audit failed: AI truth doorway={_V487_AI_TRUTH_AUDIT}")
if "Death, Grief, and the Human Search for Continuity" not in _V487_GRIEF_AUDIT: raise RuntimeError(f"USE v487.29 invariant audit failed: grief doorway={_V487_GRIEF_AUDIT}")
if "The Silent Epidemic" not in _V487_LONELINESS_AUDIT: raise RuntimeError(f"USE v487.29 invariant audit failed: loneliness doorway={_V487_LONELINESS_AUDIT}")
if not all(_V487_FOUNDATION_CONTRACT_AUDIT.values()): raise RuntimeError(f"USE v487.29 foundation calibration contract failed: {_V487_FOUNDATION_CONTRACT_AUDIT}")
if not all(_V487_AI_CONTRACT_AUDIT.values()): raise RuntimeError(f"USE v487.29 AI calibration contract failed: {_V487_AI_CONTRACT_AUDIT}")
if not all(_V487_GRIEF_CONTRACT_AUDIT.values()): raise RuntimeError(f"USE v487.29 grief calibration contract failed: {_V487_GRIEF_CONTRACT_AUDIT}")
if not all(_V487_LONELINESS_CONTRACT_AUDIT.values()): raise RuntimeError(f"USE v487.29 loneliness calibration contract failed: {_V487_LONELINESS_CONTRACT_AUDIT}")
def _v487_authoritative_documents_for_query(user_query,profile,original_documents):
    ...
    if family.get("direct"): ordered.append(family["direct"])
    ordered.extend(family.get("adjacent",[]))
    for item in ordered:
        key=(str(item.get("url") or "").strip(),str(item.get("title") or "").strip().casefold())
        if key in seen: continue
        seen.add(key); docs.append({"title":item["title"],"url":item["url"],"text":"","canonical":True,"_use_canonical_family":"ai_truth" if profile.get("ai_truth") else "grief" if profile.get("grief") else ""})
    for item in original_documents or []:
        if not isinstance(item,dict): continue
        key=(str(item.get("url") or "").strip(),str(item.get("title") or "").strip().casefold())
        if key in seen: continue
        seen.add(key); docs.append(item)
    return docs
async def _v487_query_wrapper(*args,**kwargs):
    query=_extract_user_query(args,kwargs); profile=_inquiry_profile(query); original=await _original_handle_query(*args,**kwargs)
    if isinstance(original,dict):
        docs=original.get("retrieved_documents") or original.get("documents") or []
        if profile["action"] in {"recommendation","risk"}:
            auth=_v487_authoritative_documents_for_query(query,profile,docs); original["retrieved_documents"]=auth; original["documents"]=auth
        original["visitor_boundary_version"]=APP_VERSION; original["visitor_movement_task"]=profile["action"]
    return original
app=use_core.app
...
use_core.generate_llm_response=_v487_generate_boundary
use_core._evidence_sufficiency_unavailable_response=_v487_evidence_gap_boundary
use_core.handle_query=_v487_query_wrapper
print(f"USE v487.29 ACTIVE: version={APP_VERSION}, fingerprint={DEPLOYMENT_FINGERPRINT}, core_sha={_core_runtime_sha}, source_sha256={RUNTIME_SOURCE_SHA256}")
