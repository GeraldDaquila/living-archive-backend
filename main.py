...
def fetch_canonical_context(query: str, top_k: int = 5) -> str:
    ...
    try:
        ...
        for match in query_response.get("matches", []):
            ...
            # Checks multiple common URL key aliases in vector metadata
            url = meta.get("url", meta.get("permalink", meta.get("link", meta.get("slug", "")))).strip()
...
async def run_diagnostic(request: QueryRequest):
    ...
    Exposes raw Pinecone matches, scores, index stats, and full raw metadata payload.
    """
    user_query = request.query.strip()
    if not user_query:
        raise HTTPException(status_code=400, detail="Query cannot be empty.")
