# USE PRODUCTION VERSION: v487.23 — protected authority handoff
import hashlib
import importlib
import re
from pathlib import Path
APP_VERSION="v487.23"
DEPLOYMENT_FINGERPRINT="USE-v487.23-protected-authority-handoff"
CANONICAL_BUILD_ID="USE-BUILD-v487.23-protected-authority-handoff"
EXPECTED_CORE_BLOB_SHA="fb3208a8d287f16562ffd640d89f65d5e8d18607"
_MAIN_PATH=Path(__file__).resolve(); _CORE_PATH=_MAIN_PATH.with_name("use_core.py")
RUNTIME_SOURCE_SHA256=hashlib.sha256(_MAIN_PATH.read_bytes()).hexdigest()
if not _CORE_PATH.exists(): raise RuntimeError("USE v487.23 package integrity failure: use_core.py is missing.")
_core_bytes=_CORE_PATH.read_bytes(); _core_runtime_sha=hashlib.sha1(f"blob {len(_core_bytes)}\0".encode()+_core_bytes).hexdigest()
if _core_runtime_sha!=EXPECTED_CORE_BLOB_SHA: raise RuntimeError("USE v487.23 package integrity failure: protected core mismatch.")
...
if _original_handle_query is None: raise RuntimeError("USE v487.23 package integrity failure: API query handler unavailable.")
if not callable(_original_evidence_sufficiency_unavailable_response): raise RuntimeError("USE v487.23 package integrity failure: evidence-gap boundary unavailable.")
...
def _recommendation_anchor_score(query,claim,profile):
    ...
    if claim.get("canonical"): score += 20000.0
    return score
...
def _recommendation_answer(query,docs,profile):
    claims=_extract_claims(_candidate_sentences(query,docs)); catalog=_canonical_catalog(profile)
    if profile.get("ai_truth") or profile.get("grief"):
        family_primary=_canonical_family_primary(profile); primary=_canonical_family_claim(profile); secondary=next(({"text":"","title":i["title"],"url":i["url"],"score":9000.0,"epistemic":"supported","canonical":True} for i in catalog.get("adjacent",[])),None)
    else:
...
if "Living Archive" not in _V487_FOUNDATION_AUDIT: raise RuntimeError("USE v487.23 invariant audit failed: foundation.")
if not _catalog_audit(_V487_AI_TRUTH_CATALOG,"Truth in the Age of AI: Why Discernment Is Becoming a Survival Skill"): raise RuntimeError("USE v487.23 invariant audit failed: AI truth catalog.")
if not _catalog_audit(_V487_GRIEF_CATALOG,"Death, Grief, and the Human Search for Continuity"): raise RuntimeError("USE v487.23 invariant audit failed: grief catalog.")
if "Truth in the Age of AI" not in _V487_AI_TRUTH_AUDIT: raise RuntimeError(f"USE v487.23 invariant audit failed: AI truth doorway={_V487_AI_TRUTH_AUDIT}")
if "Death, Grief, and the Human Search for Continuity" not in _V487_GRIEF_AUDIT: raise RuntimeError(f"USE v487.23 invariant audit failed: grief doorway={_V487_GRIEF_AUDIT}")
if not all(_V487_FOUNDATION_CONTRACT_AUDIT.values()): raise RuntimeError(f"USE v487.23 foundation calibration contract failed: {_V487_FOUNDATION_CONTRACT_AUDIT}")
if not all(_V487_AI_CONTRACT_AUDIT.values()): raise RuntimeError(f"USE v487.23 AI calibration contract failed: {_V487_AI_CONTRACT_AUDIT}")
if not all(_V487_GRIEF_CONTRACT_AUDIT.values()): raise RuntimeError(f"USE v487.23 grief calibration contract failed: {_V487_GRIEF_CONTRACT_AUDIT}")

def _v487_authoritative_documents_for_query(user_query, profile, original_documents):
    """Construct the protected-document authority object consumed by core.

    For calibrated AI/truth and grief families the canonical catalog is the
    destination authority. Retrieval evidence remains supporting context only.
    """
    docs=[]
    seen=set()
    family=_canonical_catalog(profile)
    ordered=[]
    ...
    for item in ordered:
        ...
        if key in seen: continue
        seen.add(key)
        docs.append({"title":item["title"],"url":item["url"],"text":"","canonical":True,"_use_canonical_family":"ai_truth" if profile.get("ai_truth") else "grief" if profile.get("grief") else ""})
...
def _v487_query_wrapper(*args,**kwargs):
    user_query=_extract_user_query(args,kwargs)
    profile=_inquiry_profile(user_query)
    print(f"The Guide v487.23 QUERY AUTHORITY: entering query handler, query={user_query!r}")
    if profile.get("ai_truth") or profile.get("grief"):
        family_docs=_v487_authoritative_documents_for_query(user_query,profile,[])
        # Bind the protected canonical family directly into the generation
        # boundary used by use_core. This is the authority object the core
        # recommendation layer consumes; it cannot be displaced by generic
        # retrieval evidence.
        kwargs=dict(kwargs)
        kwargs["protected_documents"]=family_docs
        if len(args)>=2 and isinstance(args[1],str):
            # No positional mutation is attempted; use_core receives authority
            # through the named boundary wherever supported by the wrapped path.
            pass
    result=_original_handle_query(*args,**kwargs)
    try:
        if isinstance(result,dict):
            canonical_primary=(_canonical_family_primary(profile) or {}).get("title","")
            result=dict(result)
            result["use_v48723_query_authority"]={"family":"ai_truth" if profile.get("ai_truth") else "grief" if profile.get("grief") else "general","canonical_primary":canonical_primary,"authority_bound":bool(profile.get("ai_truth") or profile.get("grief"))}
            if profile.get("ai_truth") or profile.get("grief"):
                protected_answer=(_recommendation_answer(user_query,[],profile) or "").strip()
                if protected_answer and str(result.get("response") or "").strip()!=protected_answer:
                    print(f"The Guide v487.23 QUERY AUTHORITY: replacing non-authoritative response for family={'ai_truth' if profile.get('ai_truth') else 'grief'}.")
                    result["response"]=protected_answer; result["intent"]="TOPICAL_INQUIRY"
            return result
        return result
    except Exception as exc:
        print(f"The Guide v487.23 QUERY AUTHORITY error: {exc}")
        return result

app=use_core.app; app.title=f"Find Your Way (The Guide) {APP_VERSION}"
print(f"The Guide v487.23 BUILD IDENTITY: build_id={CANONICAL_BUILD_ID}, version={APP_VERSION}, fingerprint={DEPLOYMENT_FINGERPRINT}, source_sha256={RUNTIME_SOURCE_SHA256}, core_blob_sha={_core_runtime_sha}")
