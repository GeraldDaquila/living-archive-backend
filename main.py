...
def _extract_archive_metadata(doc: dict, docs: list) -> dict:
    ...
    for candidate in docs:
        ...
        if not candidate_title or candidate_title.casefold() == title.casefold() or not re.match(r"^https?://", candidate_url, re.IGNORECASE):
            continue
...
def _archive_context(primary: dict, docs: list) -> str:
    ...
    if meta.get("related_resources"):
        resources = meta["related_resources"][:3]
        links = [f"[{r['title']}]({r['url']})" for r in resources if r.get("title") and r.get("url")]
        if links:
...
def _archive_constellation_interpretation(meta: dict, profile: dict) -> str:
    ...
    titles = [_normalize_title(r.get("title") or "") for r in resources if r.get("title")]
    if not titles:
        return ""
...
def _archive_bridge(profile: dict, meta: dict, secondaries: list) -> str:
    resources = meta.get("related_resources") or []
    titles = [_normalize_title(r.get("title") or "") for r in resources if r.get("title")]
    secondary_corpora = " ".join(f"{_normalize_title(doc.get('title') or '')} {re.sub(r'\s+', ' ', str(doc.get('text') or '').strip())}" for doc in secondaries).casefold()
...
def _v339_build_compassionate_recommendation_answer(user_query: str, primary: dict, contextual_docs: list) -> str:
    ...
    if architecture["archive_context"]:
        sections.append(architecture["archive_context"] + ("" if architecture["archive_context"].endswith(".") else "."))
    ...
    care = "The Archive can offer reflection and orientation, but where there is immediate danger or coercion, the next step should be real-world safety and trusted human support rather than reflection alone." if profile["risk"] else "You do not need to agree with every idea in these pieces. In grief, it can be enough to find a thought that gives you some companionship, some language for what you are carrying, or simply a place to pause. Take what feels useful, leave what does not, and let the questions remain open where they need to." if profile["grief"] else "Take what feels useful, leave what does not, and let the question remain open where it needs to."
...
def _v340_orientation_boundary(user_query: str, answer: str, retrieved_context_blocks: str) -> str:
    ...
    if not candidates:
        return value
    low = value.casefold()
    fallback_signals = ("does not establish a reliable explanation", "does not establish a next destination", "question remains open", "closest places surfaced", "relevant material available for your question", "could not complete its interpretive response")
