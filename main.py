...
_SUBJECT_STOPWORDS = frozenset({
    ...
    "struggling", "struggle", "dealing", "myself", "finding", "it", "its", "all", "very", "just", "really", "more",
    "less", "ever", "often", "sometimes", "now", "today", "here", "there", "something", "anything",
})
...
def _role_evidence(doc):
    ...
    return {"worldview":bool(worldview_in_title or worldview_early_hits>=2),"risk":bool(risk_in_title or risk_early_hits>=1),"abuse":bool(abuse_in_title or abuse_early_hits>=1)}
...
def _contextual_fit(query,doc):
    ...
    text=str(doc.get("text") or doc.get("content") or doc.get("excerpt") or "").casefold()
    early=text[:3000]
    title_tokens=set(re.findall(r"[a-z0-9]+",title)); early_tokens=set(re.findall(r"[a-z0-9]+",early))
...
def _eligible_outward_doc(query, doc, profile, *, min_relevance=2):
    ...
    if not title or not re.match(r"^https://\S+$",url,re.I): return False
...
# Structural bridge: recommendation/navigation requests are not evidence-gap
# terminal states. If canonical evidence exists, reuse that evidence as the
# generation context so the visitor boundary can select a grounded doorway.
...
def _sanitize_outward_context(query, docs, profile):
    # One boundary governs every resource that can leave the system: primary,
    # adjacent, evidence-gap, fallback, and closest-place navigation.
    eligible=[]
    for doc in docs or []:
...
# Startup audit: the exact failure class is reproduced. A risk decoy is
# rejected, a directly evidenced anger resource wins, and the evidence-gap
