# USE PRODUCTION VERSION: v356 — transition/open-question evidence boundary
# v356 preserves v355's deterministic Guide paths while adding a bounded recovery
# and evidence-fit gate for open life-transition questions. Protected use_core.py unchanged.
import hashlib
import importlib
import re
...
APP_VERSION = "v356"
DEPLOYMENT_FINGERPRINT = "USE-v356-transition-evidence-boundary"
CANONICAL_BUILD_ID = "USE-BUILD-v356-transition-evidence-boundary"
...
if not _CORE_PATH.exists():
    raise RuntimeError("USE v356 package integrity failure: use_core.py is missing.")
_core_runtime_sha = _git_blob_sha256(_CORE_PATH.read_bytes())
if _core_runtime_sha != EXPECTED_CORE_BLOB_SHA:
    raise RuntimeError(
        f"USE v356 package integrity failure: expected protected core blob sha={EXPECTED_CORE_BLOB_SHA}, actual={_core_runtime_sha}"
    )
...
def _archive_context(primary, docs, profile, primary_title):
    ...
    if profile.get("afterlife"):
        ...
        related_docs = [doc for doc in related_docs if _transition_evidence_fit(doc, profile) >= 4][:2]
...
def _guide_answer(user_query, primary, docs):
    ...
    elif (profile.get("sensitive") or profile.get("transition")) and profile.get("open_question"):
...
def _transition_primary_from_docs(user_query, docs):
    ...
    scored = []
    for i, doc in enumerate(docs):
        fit = _transition_evidence_fit(doc, profile)
        if fit >= 5 and str(doc.get("url") or "").strip() and _normalize_title(doc.get("title") or ""):
...
def _recover_transition_candidates(user_query):
    ...
    if not callable(retriever):
        print("USE v356 transition recovery: protected function-targeted retrieval unavailable.")
        return []
    try:
        recovered = retriever(user_query)
    except Exception as exc:
        print(f"USE v356 transition recovery error: {type(exc).__name__}: {exc}")
        return []
    if not isinstance(recovered, list):
        return []
    print(f"USE v356 transition recovery: protected_function_candidates={len(recovered)}")
    return recovered


def _transition_evidence_gap_response():
    return (
        "The Living Archive does not currently have sufficiently grounded canonical material "
        "for this particular question, and I do not want to point you to a resource merely "
        "because its wording happens to overlap. It is better to leave the doorway open than "
        "pretend an unrelated resource is the right place to begin."
    )


def _v356_finalize(*args, **kwargs):
    ...
    if not recommendation and (not intent or intent == "TOPICAL_INQUIRY"):
        ...
        if primary:
            print(f"USE v356 GUIDE GENERATION SHORT-CIRCUIT: primary='{_normalize_title(primary.get('title') or '')}', provider_generation_skipped=True")
        ...
        if profile.get("transition") and profile.get("open_question"):
            ...
            if primary:
                print(f"USE v356 TRANSITION EVIDENCE GATE: primary='{_normalize_title(primary.get('title') or '')}', provider_generation_skipped=True")
                return _guide_answer(query, primary, fit_docs)
            print("USE v356 TRANSITION EVIDENCE GATE: no sufficiently aligned canonical evidence; provider_generation_skipped=True")
            return _transition_evidence_gap_response()

        primary = _sensitive_open_question_can_use_guide(query, docs)
        if primary:
            print(f"USE v356 GUIDE GENERATION SHORT-CIRCUIT: primary='{_normalize_title(primary.get('title') or '')}', provider_generation_skipped=True, posture=sensitive_open_question")
            return _guide_answer(query, primary, docs)
...
use_core.generate_llm_response = _v356_finalize
print(f"USE v356 GUIDE BUILD IDENTITY: build_id={CANONICAL_BUILD_ID}, version={APP_VERSION}, fingerprint={DEPLOYMENT_FINGERPRINT}, source_sha256={RUNTIME_SOURCE_SHA256}, core_blob_sha256={_core_runtime_sha}")
