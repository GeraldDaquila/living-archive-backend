# USE PRODUCTION VERSION: v359 — transition retrieval strategy bridge
import hashlib
import importlib
import re
...
APP_VERSION = "v359"
DEPLOYMENT_FINGERPRINT = "USE-v359-transition-retrieval-strategy"
CANONICAL_BUILD_ID = "USE-BUILD-v359-transition-retrieval-strategy"
...
if not _CORE_PATH.exists(): raise RuntimeError("USE v359 package integrity failure: use_core.py is missing.")
_core_runtime_sha = _git_blob_sha256(_CORE_PATH.read_bytes())
if _core_runtime_sha != EXPECTED_CORE_BLOB_SHA:
    raise RuntimeError(f"USE v359 package integrity failure: expected protected core blob sha={EXPECTED_CORE_BLOB_SHA}, actual={_core_runtime_sha}")
...
def _transition_primary_from_docs(user_query,docs):
    ...
    for i,doc in enumerate(docs):
        ...
        if fit>=6 and clusters["transition"] and clusters["meaning"] and clusters["experience"] and clusters["grounding"] and str(doc.get("url") or "").strip() and _normalize_title(doc.get("title") or ""):
...
def _transition_retrieval_strategy(user_query):
    """Expand the existing protected retrieval strategy for open transition questions.

    This uses the protected function-targeted retrieval abstraction with an explicit
    visitor-question/function profile, plus one additional question-shaped semantic
    pass. It never names, maps, or inserts a canonical resource. All candidates still
    originate from the canonical index and pass through the existing downstream gates.
    """
    ...
    if callable(retriever):
        try:
            ...
            print(f"USE v359 transition strategy: function-targeted retrieval error: {type(exc).__name__}: {exc}")
            recovered = []
    print(f"USE v359 transition strategy: protected_function_candidates={len(recovered) if isinstance(recovered,list) else 0}")
    ...
    if index is not None and callable(embed) and callable(query_index):
        ...
        for variant in variants:
            try:
                ...
                for _score,_match_id,metadata in query_index(vector, min(max(getattr(use_core,"RETRIEVAL_TOP_K",12)*2,24),48)):
                    if isinstance(metadata,dict): semantic_candidates.append(metadata)
            except Exception as exc:
                print(f"USE v359 transition strategy: semantic recovery error: {type(exc).__name__}: {exc}")
    print(f"USE v359 transition strategy: semantic_recovery_candidates={len(semantic_candidates)}")
    return _merge_recovered_documents([], list(recovered if isinstance(recovered,list) else []) + semantic_candidates)
...
def _guide_answer(user_query,primary,docs):
    ...
    if profile.get("transition"):
        fit_docs=[d for d in docs if _transition_evidence_fit(d,profile)[0]>=5]
        roles=[]
        seen=set()
        for d in fit_docs:
            role=_secondary_role(d,profile)
            if role in seen: continue
            link=_resource_link(d)
            if link and _normalize_title(d.get("title") or "").casefold()!=title.casefold():
...
def _v359_finalize(*args,**kwargs):
    ...
    if not recommendation and (not intent or intent=="TOPICAL_INQUIRY"):
        primary=getattr(use_core,"_find_primary",None)
        meaning=getattr(use_core,"_meaning_question_can_use_guide",None)
        grief=getattr(use_core,"_grief_question_can_use_guide",None)
        selected=(primary(query,docs) if callable(primary) else None) or (meaning(query,docs) if callable(meaning) else None) or (grief(query,docs) if callable(grief) else None)
        ...
        if profile.get("transition") and profile.get("open_question"):
            ...
            selected,fit_docs=_transition_primary_from_docs(query,merged)
            if selected:
                print(f"USE v359 TRANSITION EVIDENCE GATE: primary='{_normalize_title(selected.get('title') or '')}', provider_generation_skipped=True")
                return _guide_answer(query,selected,fit_docs)
            print("USE v359 TRANSITION EVIDENCE GATE: no sufficiently aligned canonical evidence; provider_generation_skipped=True")
...
use_core.APP_VERSION=APP_VERSION; use_core.DEPLOYMENT_FINGERPRINT=DEPLOYMENT_FINGERPRINT; use_core.CANONICAL_BUILD_ID=CANONICAL_BUILD_ID; use_core.generate_llm_response=_v359_finalize
print(f"USE v359 GUIDE BUILD IDENTITY: build_id={CANONICAL_BUILD_ID}, version={APP_VERSION}, fingerprint={DEPLOYMENT_FINGERPRINT}, source_sha256={RUNTIME_SOURCE_SHA256}, core_blob_sha256={_core_runtime_sha}")