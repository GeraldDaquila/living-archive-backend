# USE PRODUCTION VERSION: v487.30 — structural experiential/recommendation routing repair
import hashlib
import importlib
import re
...
APP_VERSION="v487.30"
DEPLOYMENT_FINGERPRINT="USE-v487.30-structural-runtime-contract-repair"
CANONICAL_BUILD_ID="USE-BUILD-v487.30-structural-runtime-contract-repair"
EXPECTED_CORE_BLOB_SHA="fb3208a8d287f16562ffd640d89f65d5e8d18607"
_MAIN_PATH=Path(__file__).resolve()
RUNTIME_SOURCE_SHA256=hashlib.sha256(_MAIN_PATH.read_bytes()).hexdigest()
if getattr(_base,"_core_runtime_sha","")!=EXPECTED_CORE_BLOB_SHA:
    raise RuntimeError("USE v487.30 package integrity failure: protected core mismatch.")
...
# Runtime contract adapter: v487.29 introduced a dependency on a rationale hook
# that was not actually exported by the preserved v487.28 runtime. Keep the
# response architecture single-sourced by defining the contract at the overlay
# boundary and explicitly exporting it to the preserved runtime before use.
def _recommendation_rationale(primary,profile):
    title=str((primary or {}).get("title") or "the material")
    if profile.get("grief"):
        return f"I’m recommending this first because it approaches grief and the human search for continuity directly, making it a more immediate place to reflect on the experience of losing someone you love."
    if profile.get("ai_truth"):
        return f"I’m recommending this first because it gives you a direct place to examine discernment and the question of how we decide what is actually true."
    return f"I’m recommending this first because it offers a direct place to reflect on the question you brought here, without asking you to treat it as the whole answer."
...
if _profile["action"]!="recommendation":
    raise RuntimeError(f"USE v487.30 invariant failed: loneliness action={_profile['action']}")
...
if _probe_mode!="recommendation" or "The Silent Epidemic" not in _probe_answer or "reflection gateway" not in _probe_answer:
    raise RuntimeError(f"USE v487.30 invariant failed: loneliness visitor contract mode={_probe_mode}, answer={_probe_answer}")
...
if _base._inquiry_profile(_ai)["action"]!="recommendation": raise RuntimeError("USE v487.30 invariant failed: AI movement task")
if _base._inquiry_profile(_grief)["action"]!="recommendation": raise RuntimeError("USE v487.30 invariant failed: grief movement task")
...
print(f"USE v487.30 ACTIVE: version={APP_VERSION}, fingerprint={DEPLOYMENT_FINGERPRINT}, core_sha={getattr(_base,'_core_runtime_sha','')}, source_sha256={RUNTIME_SOURCE_SHA256}")
