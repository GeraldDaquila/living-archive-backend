# USE PRODUCTION VERSION: v487.14 — canonical doorway family gate
import hashlib
import importlib
import re
from pathlib import Path
APP_VERSION="v487.14"
DEPLOYMENT_FINGERPRINT="USE-v487.14-canonical-doorway-family-gate"
CANONICAL_BUILD_ID="USE-BUILD-v487.14-canonical-doorway-family-gate"
EXPECTED_CORE_BLOB_SHA="fb3208a8d287f16562ffd640d89f65d5e8d18607"
_MAIN_PATH=Path(__file__).resolve(); _CORE_PATH=_MAIN_PATH.with_name("use_core.py")
RUNTIME_SOURCE_SHA256=hashlib.sha256(_MAIN_PATH.read_bytes()).hexdigest()
if not _CORE_PATH.exists(): raise RuntimeError("USE v487.14 package integrity failure: use_core.py is missing.")
_core_bytes=_CORE_PATH.read_bytes(); _core_runtime_sha=hashlib.sha1(f"blob {len(_core_bytes)}\0".encode()+_core_bytes).hexdigest()
if _core_runtime_sha!=EXPECTED_CORE_BLOB_SHA: raise RuntimeError("USE v487.14 package integrity failure: protected core mismatch.")
...
if _original_handle_query is None: raise RuntimeError("USE v487.14 package integrity failure: API query handler unavailable.")
if not callable(_original_evidence_sufficiency_unavailable_response): raise RuntimeError("USE v487.14 package integrity failure: evidence-gap boundary unavailable.")
...
def _canonical_role(title,text,profile):
    ...
    if profile.get("ai_truth"):
        if re.search(r"\b(?:truth in the age of ai|why discernment is becoming a survival skill|discernment.*survival skill)\b",low): return "ai_truth_direct"
        if re.search(r"\b(?:knowledge stewardship.*ai|ai era.*information to wisdom|information to wisdom.*ai)\b",low): return "ai_truth_adjacent"
        if re.search(r"\b(?:meaning crisis.*artificial intelligence|artificial intelligence.*meaning crisis|ai.*meaning|ai.*identity|identity.*ai)\b",low): return "ai_truth_adjacent"
        if re.search(r"\b(?:living archive navigator|network architecture|regeneration)\b",low): return "ai_truth_structural"
    if profile.get("grief"):
        if re.search(r"\b(?:death, grief|grief.*continuity|meaning in grief|transformative power of loss|grief.*loss)\b",low): return "grief_direct"
        if re.search(r"\b(?:afterlife|reincarnation|near-death|hypnosis)\b",low): return "grief_specialized"
    return "general"
def _recommendation_anchor_score(query,claim,profile):
    ...
    if profile.get("ai_truth"):
        family_bonus={"ai_truth_direct":1000,"ai_truth_adjacent":500,"ai_truth_structural":-500,"general":-50}.get(role,-50)
        score+=family_bonus
        if role=="ai_truth_direct": score+=100
        elif role=="ai_truth_adjacent": score+=40
...
def _recommendation_answer(query,docs,profile):
    ...
    if profile.get("ai_truth"):
        direct=[c for c in claims if _canonical_role(c["title"],c["text"],profile)=="ai_truth_direct"]
        adjacent=[c for c in claims if _canonical_role(c["title"],c["text"],profile)=="ai_truth_adjacent"]
        direct.sort(key=lambda c:_recommendation_anchor_score(query,c,profile),reverse=True)
        adjacent.sort(key=lambda c:_recommendation_anchor_score(query,c,profile),reverse=True)
        primary=direct[0] if direct else (adjacent[0] if adjacent else None)
        secondary=adjacent[0] if adjacent and adjacent[0] is not primary else None
    else:
        ranked=sorted(claims,key=lambda c:_recommendation_anchor_score(query,c,profile),reverse=True)
        primary=ranked[0] if ranked else None; secondary=ranked[1] if len(ranked)>1 else None
...
if "Living Archive" not in _V487_FOUNDATION_AUDIT: raise RuntimeError("USE v487.14 invariant audit failed: foundation.")
if "Truth in the Age of AI" not in _V487_AI_TRUTH_AUDIT: raise RuntimeError(f"USE v487.14 invariant audit failed: AI truth doorway={_V487_AI_TRUTH_AUDIT}")
if not all(_V487_FOUNDATION_CONTRACT_AUDIT.values()): raise RuntimeError(f"USE v487.14 foundation calibration contract failed: {_V487_FOUNDATION_CONTRACT_AUDIT}")
if not all(_V487_CONTRACT_AUDIT.values()): raise RuntimeError(f"USE v487.14 recommendation calibration contract failed: {_V487_CONTRACT_AUDIT}")
app=use_core.app; app.title=f"Find Your Way (The Guide) {APP_VERSION}"
print(f"The Guide v487.14 BUILD IDENTITY: build_id={CANONICAL_BUILD_ID}, version={APP_VERSION}, fingerprint={DEPLOYMENT_FINGERPRINT}, source_sha256={RUNTIME_SOURCE_SHA256}, core_blob_sha={_core_runtime_sha}")
