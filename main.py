# USE PRODUCTION VERSION: v487.35 — adjacent path directness
import hashlib
import importlib
import re
...
APP_VERSION="v487.35"
DEPLOYMENT_FINGERPRINT="USE-v487.35-adjacent-path-directness"
CANONICAL_BUILD_ID="USE-BUILD-v487.35-adjacent-path-directness"
EXPECTED_CORE_BLOB_SHA="fb3208a8d287f16562ffd640d89f65d5e8d18607"
_MAIN_PATH=Path(__file__).resolve()
RUNTIME_SOURCE_SHA256=hashlib.sha256(_MAIN_PATH.read_bytes()).hexdigest()
if getattr(_base,"_core_runtime_sha","")!=EXPECTED_CORE_BLOB_SHA:
    raise RuntimeError("USE v487.35 package integrity failure: protected core mismatch.")
...
_EXPERIENTIAL_STATE_TERMS=(
    r"lonelin(?:ess|e)",r"empt(?:iness|y)",r"sad(?:ness|ly)",r"sorrow",
    r"despair",r"hopeless(?:ness)?",r"anxiety",r"anxious",r"fear",r"afraid",
    r"anger",r"angry",r"resentment",r"shame",r"guilt",r"confusion",
    r"uncertain(?:ty)?",r"isolation",r"isolated",r"disconnection",
    r"disconnected",r"heartbreak",r"breakup",r"betrayal",r"burnout",
    r"stress",r"overwhelmed",r"grief",r"grieving",r"bereavement",
    r"mourning",r"loss",r"relationship",r"abuse",r"trauma",
    r"forgiveness",r"letting\s+go"
)
def _has(q,patterns):
...
_RECOMMENDATION_GENERIC_TERMS=frozenset({
    "anything","something","might","may","could","would","should","help","think","thought",
    "thinking","about","question","questions","living","archive","site","website","guide",
    "place","begin","start","first","read","reading","explore","exploring","reflect",
    "reflection","recommend","recommendation","suggest","suggestion","advice","advise",
    "essay","essays","article","articles","resource","resources","piece","pieces","material",
    "where","what","which","how","why","can","please","find","give","offer","tell","one",
    "best","good","for","from","with","into","through","there","here","someone","need",
    "want","looking","thing","things"
})
_RECOMMENDATION_GENERIC_STANCE_TERMS=frozenset({
    "i","im","am","struggling","struggle","dealing","having","hard","time","going",
    "experiencing","feeling","feel","worried","worry","afraid","confused","unsure","sure",
    "keep","still","know","don't","dont","not","my","me","mine","it","its","this","that"
})
...
def _recommendation_term_variants(term):
    ...
    if value.endswith("iness") and len(value)>6:
        variants.add(value[:-5]+"y")
...
def _generic_recommendation_subject_terms(query):
    ...
    for token in tokens:
        clean=token.strip("-'")
        if len(clean)<3 or clean in _RECOMMENDATION_GENERIC_TERMS or clean in _RECOMMENDATION_GENERIC_STANCE_TERMS:
            continue
...
def _generic_recommendation_directness(query,doc):
    ...
    if not title or not subject_terms:
        return (0,0,0,0,0)
    ...
    phrase_hits=sum(
        1 for i in range(len(subject_terms)-1)
        if f"{subject_terms[i]} {subject_terms[i+1]}" in title
        or f"{subject_terms[i]} {subject_terms[i+1]}" in text[:2400]
    )
...
def _canonical_primary_from_docs(canonical_docs,query,profile):
    docs=[
        d for d in (canonical_docs or [])
        if isinstance(d,dict)
        and str(d.get("title") or "").strip()
        and str(d.get("url") or "").startswith("https://")
    ]
    if not docs:
        return None
    scored=[]
    ...
    return {
        ...
        "epistemic":"supported",
        "canonical":True,
        "_authority":"visitor_canonical_generic_directness_adjudication"
    }

def _recommendation_secondary_key(candidate,query,index):
    title=str(candidate.get("title") or "")
    directness=_generic_recommendation_directness(query,{
        "title":title,
        "text":candidate.get("text","") or candidate.get("content","") or candidate.get("excerpt","") or "",
        "url":candidate.get("url","")
    })
    anchor=float(_base._recommendation_anchor_score(query,candidate,_base._inquiry_profile(query)))
    evidence=float(candidate.get("score",0) or 0)
    return (directness,anchor,evidence,-index)
...
def _recommendation_answer_with_authority(query,docs,profile,canonical_docs=None):
    ...
    if authoritative:
        ...
        ranked=[
            c for c in claims
            if str(c.get("title") or "").casefold()!=primary["title"].casefold()
        ]
        secondary=max(
            ranked,
            key=lambda c:_recommendation_secondary_key(c,query,ranked.index(c)),
            default=None
        )
    else:
        ranked=sorted(
            claims,
            key=lambda c:_recommendation_secondary_key(c,query,claims.index(c)),
            reverse=True
        )
    ...
    parts=[
        f"A useful place to begin with this question is {primary_link}",
        _generic_recommendation_wisdom(profile),
    ...
    if secondary:
        parts.append(
            f"A nearby path is [{secondary['title']}]({secondary['url']}), "
            "which opens another aspect of the question without asking you to treat either doorway as the whole answer."
        )
...
# Four-pass startup audit: primary directness must defeat a decoy, and the
# adjacent path must be selected by relevance rather than raw transport order.
...
if _probe_profile["action"]!="recommendation":
    raise RuntimeError(f"USE v487.35 invariant failed: loneliness action={_probe_profile['action']}")
_probe_canonical=[
...
_probe_retrieved=[
    {"title":"You Are Enough: Freeing Inner Beauty from the Clutches of Expectations","url":"https://geralddaquila.com/2025/06/01/you-are-enough-freeing-inner-beauty-from-the-clutches-of-expectations/","text":"Expectations can shape how people understand themselves."}
]
...
if _probe_mode!="recommendation" or not _probe_answer.startswith("A useful place to begin with this question is [The Silent Epidemic:"):
    raise RuntimeError(f"USE v487.35 invariant failed: canonical directness adjudication: {_probe_answer}")
...
if _base._inquiry_profile(_ai)["action"]!="recommendation":
    raise RuntimeError("USE v487.35 invariant failed: AI movement task")
if _base._inquiry_profile(_grief)["action"]!="recommendation":
    raise RuntimeError("USE v487.35 invariant failed: grief movement task")
...
print(f"USE v487.35 ACTIVE: version={APP_VERSION}, fingerprint={DEPLOYMENT_FINGERPRINT}, core_sha={getattr(_base,'_core_runtime_sha','')}, source_sha256={RUNTIME_SOURCE_SHA256}")
