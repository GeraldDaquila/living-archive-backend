# USE PRODUCTION VERSION: v290 — Embedding Runtime Thread Optimization: Preserve v288 Provider + Model + Retrieval Behavior + The Guide
...
# deterministic.
DEPLOYMENT_FINGERPRINT = "USE-v290-embedding-runtime-default-threads"

# === CANONICAL BUILD IDENTITY (excluded from payload hash) ===
CANONICAL_BUILD_ID = "USE-BUILD-v290-embedding-runtime-default-threads"
CANONICAL_BUILD_PAYLOAD_SHA256 = "e841f8e097e1cb0cd5d9242ad61b361c7ed6fc83ee43cf7dc8482f6bfa680c4f"
...
# v290 retains provider/runtime observability from v288. This reads the
...
try:
    ...
    print(
        "USE v290 embedding-runtime diagnostic: "
...
def generate_embedding(text: str, diagnostic_label: Optional[str] = None) -> List[float]:
    # v290 diagnostic is opt-in so non-diagnostic callers retain the same
    ...
    try:
        ...
        if not embeddings:
            ...
            if diagnostic_label:
                print(
                    "USE v290 embedding-provider diagnostic: "
...
def _function_targeted_candidate_search(question: str) -> List[Dict[str, Any]]:
    ...
    for function_name in ordered_targets[:target_limit]:
        ...
        print(
            "USE v290 function-targeted timing: "
...
def fetch_canonical_context(
    ...
    # v290 inherited fetch-stage diagnostic: preserve the previously decomposed
    ...
    _v290_postretrieval_started = time.perf_counter()
    ...
    print(
        "USE v290 fetch diagnostic: "
        f"canonical_evidence_stage={time.perf_counter() - _v290_postretrieval_started:.3f}s"
    )
    _v290_stage_started = time.perf_counter()
    ...
    print(
        "USE v290 fetch diagnostic: "
        f"authority_and_doorway_stage={time.perf_counter() - _v290_stage_started:.3f}s"
    )
    _v290_stage_started = time.perf_counter()
    ...
    print(
        "USE v290 fetch diagnostic: "
        f"post_selection_gates_stage={time.perf_counter() - _v290_stage_started:.3f}s"
    )
    _v290_stage_started = time.perf_counter()
    ...
    print(
        "USE v290 fetch diagnostic: "
        f"evidence_preservation_stage={time.perf_counter() - _v290_stage_started:.3f}s"
    )
    _v290_stage_started = time.perf_counter()
    ...
    print(
        "USE v290 fetch diagnostic: "
        f"generation_evidence_selection_stage={time.perf_counter() - _v290_stage_started:.3f}s"
    )
    _v290_stage_started = time.perf_counter()
    ...
    print(
        "USE v290 fetch diagnostic: "
        f"context_formatting_stage={time.perf_counter() - _v290_stage_started:.3f}s, "
        f"postretrieval_total={time.perf_counter() - _v290_postretrieval_started:.3f}s"
    )
    print(
        "USE v290 latency: "
