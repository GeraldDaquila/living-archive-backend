...
APP_VERSION = "v342"
DEPLOYMENT_FINGERPRINT = "USE-v342-question-axis-doorway"
CANONICAL_BUILD_ID = "USE-BUILD-v342-question-axis-doorway"
...
if not _CORE_PATH.exists():
    raise RuntimeError("USE v342 package integrity failure: use_core.py is missing.")
_core_runtime_sha = _git_blob_sha256(_CORE_PATH.read_bytes())
if _core_runtime_sha != EXPECTED_CORE_BLOB_SHA:
    raise RuntimeError(f"USE v342 package integrity failure: expected protected core blob sha={EXPECTED_CORE_BLOB_SHA}, actual={_core_runtime_sha}")
...
def _v342_question_axis_terms(user_query: str):
    ...
    axes=[]
    if re.search(r"\b(?:death|afterlife|reincarnation|mortality|what comes after|what happens after)\b",q):
        axes.append("mortality_afterlife")
    if re.search(r"\b(?:belief|spiritual|religious|mystical|soul|continuity|what may endure)\b",q):
        axes.append("belief_continuity")
    if re.search(r"\b(?:science|scientific|psychological|research|clinical|neuroscientific)\b",q):
        axes.append("scientific_grounding")
    if re.search(r"\b(?:meaning|purpose|identity|understanding)\b",q):
        axes.append("meaning_identity")
    return axes
...
def _v342_primary_question_fit(user_query: str, primary: dict) -> tuple:
    ...
    terms=re.findall(r"[a-z0-9]+(?:[-'][a-z0-9]+)?",re.sub(r"\s+"," ",str(user_query or "").casefold()))
    stop={"what","where","when","why","how","might","come","after","like","without","being","pushed","toward","particular","answer","begin","beginning","start","i","ive","id","dont","know","whether","believe","want","would","could","should","this","that","the","and","or","to","in","an","a","it","for","of","on","from","with","but","explore","question","something","anything"}
    qterms={t for t in terms if len(t)>=4 and t not in stop}
    corpus_tokens=set(re.findall(r"[a-z0-9]+(?:[-'][a-z0-9]+)?",content))
    direct=sum(1 for term in qterms if term in corpus_tokens or (len(term)>5 and term[:-1] in corpus_tokens))
    axes=_v342_question_axis_terms(user_query)
    axis_hits=0
    axis_patterns={
        "mortality_afterlife":r"\b(?:death|afterlife|reincarnation|mortality)\b",
        "belief_continuity":r"\b(?:belief|spiritual|religious|mystical|soul|continuity)\b",
        "scientific_grounding":r"\b(?:science|scientific|psychological|research|clinical|neuroscientific)\b",
        "meaning_identity":r"\b(?:meaning|purpose|identity|understanding)\b",
    }
    for axis in axes:
...
def _v342_select_axis_proportionate_primary(user_query: str, docs: list):
    ...
    for index,doc in enumerate(docs):
        ...
        if fit:
            profiled.append((axis_hits,direct,-index,doc))
...
def _v342_axis_proportionate_orientation(user_query: str, retrieved_context_blocks: str, canonical_primary: dict = None) -> str:
    ...
    if primary:
        ...
        matched=next((doc for doc in docs if str(doc.get("url") or "").strip()==primary_url),None)
...
def _v341_orientation_boundary(user_query:str,answer:str,retrieved_context_blocks:str,canonical_primary:dict=None)->str:
    ...
    oriented=_v342_axis_proportionate_orientation(query,retrieved_context_blocks,canonical_primary)
    if oriented and oriented.strip()!=value.strip():
        print("USE v342 question-axis Guide boundary: using a content-proportionate primary from the existing canonical evidence set.")
        return oriented
    return value
...
def _v339_finalize_generation_response(*args,**kwargs):
    ...
    if not use_core._is_recommendation_question(user_query):
        retrieved_context=_context_blocks_from_kwargs(args,kwargs); oriented=_v341_orientation_boundary(user_query,str(value or ""),retrieved_context,canonical_primary)
...
print(f"USE v342 QUESTION-AXIS DOORWAY: build_id={CANONICAL_BUILD_ID}, version={APP_VERSION}, fingerprint={DEPLOYMENT_FINGERPRINT}, source_sha256={RUNTIME_SOURCE_SHA256}, core_blob_sha256={_core_runtime_sha}")
...
_existing_v341_generate = use_core.generate_llm_response
...
def _explicit_guide_handoff_generate(*args, **kwargs):
    ...
    return _existing_v341_generate(*args,**kwargs)
