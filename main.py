...
def _select_recommendation_adjacent_anchor(query,primary,claims,profile):
    ranked=[(_adjacent_anchor_score(query,primary,c,profile),c) for c in claims if c.get("url")!=primary.get("url")]
    ranked.sort(key=lambda x:x[0],reverse=True); return ranked[0][1] if ranked and ranked[0][0]>18 else None
...
if "living archive" not in _V487_FOUNDATION_AUDIT.casefold(): raise RuntimeError("USE v487.8 invariant audit failed: foundation.")
