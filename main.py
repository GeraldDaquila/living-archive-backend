...
def _transition_retrieval_strategy(query: str):
    ...
    for doc in list(retrieved or []):
        if isinstance(doc, dict) and _is_query_aligned_transition_doorway(doc, query): ranked.append((_transition_doorway_score(doc, query), doc))
...
def _v386_finalize(*args, **kwargs):
    ...
    if tprofile["transition"] and tprofile["open_question"] and not tprofile["explicit_framework"]:
        ...
        aligned = _merge_recovered_documents([d for d in docs if _is_query_aligned_transition_doorway(d, user_query)], recovered)
    ...
    if tprofile["explicit_framework"]:
    ...
    if not value or not use_core._is_recommendation_question(user_query): return value
    try:
        governed = _original_recommendation_output_authority(user_query, value, raw_context) if _original_recommendation_output_authority else value
        if governed and _original_recommendation_resource_identity:
            value = _original_recommendation_resource_identity(user_query, governed, raw_context)
    except Exception:
        pass
    return value.strip()
