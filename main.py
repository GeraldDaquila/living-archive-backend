...
def _role_evidence(doc):
    ...
    corpus = title + " " + text
    return {
        "worldview": bool(re.search(r"\b(?:spiritual|spirituality|religious|religion|mystical|mysticism|afterlife|reincarnation|soul|sacred|transcenden|starseed|higher-order intelligence|metaphysics|cosmic curriculum|universe|cosmic|oversoul)\b", corpus)),
        "risk": bool(re.search(r"\b(?:suicid(?:e|al|ality)|self-harm|overdose|acute crisis|crisis intervention|immediate danger)\b", corpus)),
    }

def _is_risk_related(doc):
    return _role_evidence(doc)["risk"]
...
def _build_risk_answer(query):
    ...
    if re.search(r"\b(?:suicid\w*|self-harm|self harm|kill(?:ing)? myself|kill(?:ing)? yourself|want(?:ing)? to die|end my life|take my own life|harm myself|hurt myself)\b", q):
        return "If you are thinking about killing yourself or may act on thoughts of self-harm, please seek human help now. Call emergency services or go to the nearest emergency department, and if you can, stay with another person while you get help."
...
def _concept_sentences(query, docs):
    ...
    for doc in docs:
        ...
        for sentence in [s.strip() for s in re.split(r"(?<=[.!?])\s+", raw) if s.strip()]:
            ...
            if re.search(r"\b(?:collective awakening|profound truth|we'?re all co-creators|universal knowledge|quantum thread|hypothetical)\b", low):
                continue
            score = 0
...
def _plain_language_concept(subject, candidates):
    if subject.casefold() == "overflow":
        return "In the Archive's framing, Overflow is a way of understanding how life, meaning, and stewardship can be cultivated and passed onward rather than treated as something to accumulate or possess."
    if not candidates:
        return ""
...
def _build_factual_answer(query, docs):
    ...
    parts.append("A useful next step is to read the linked anchor and see which part of the concept is most relevant to your question.")
    return "\n\n".join(parts)

def _build_foundation_answer():
...
def _unified_visitor_construction(query, retrieved_docs, canonical_docs):
    ...
    if profile["foundation"]:
        return _build_foundation_answer(), "foundation"
    if profile["factual"]:
...
def _boundary_context(args, kwargs):
    query = _extract_user_query(args, kwargs)
