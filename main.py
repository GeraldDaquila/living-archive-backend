...
async def query_archive(request: QueryRequest):
    ...
    system_prompt = (
        ...
        "1. PRIORITIZE NAVIGATION FOR ENTRY QUESTIONS: When a visitor asks where to start or expresses feeling overwhelmed (e.g., 'I'm overwhelmed by how much is on this site. Where should I start?'), interpret this strictly as a request for an entry point into the corpus. Do NOT explain the Living Archive's methodology, philosophy, or USE's internal mechanics unless explicitly asked.\n"
        "2. CANONICAL ORIENTATION FIRST: Prefer an architectural or foundational orientation entry point over random substantive essays. If the retrieved context contains a canonical starting page or orientation resource, prioritize that as the primary recommendation.\n"
        "3. GROUNDED JUSTIFICATION: Only select destinations from the actual CANONICAL CONTEXT. Explain briefly and specifically why the selected entry point addresses the visitor's state or question. Never invent generic topics or fake thematic bridges.\n"
        "4. FOCUSED PATHWAY (NO DUMPING): Do NOT provide multiple unrelated choices merely to demonstrate corpus breadth. Offer the best starting point, justify it, and optionally offer a single logical next step.\n"
        "5. IDENTITY QUESTIONS EXCEPTION: If the user explicitly asks 'What is the Living Archive?', answer what it is, what it is for, and how it differs from a blog/library before giving orientation links.\n"
        ...
        "1. Recognize the Visitor's Need: Briefly acknowledge their orientation need with warmth and simplicity.\n"
        "2. Best Starting Point: Recommend the single best canonical entry point retrieved from the context.\n"
        "3. Clear Justification: Explain in 1-2 sentences why this specific page helps them get their bearings.\n"
        "4. Single Next Step (Optional): Offer one natural follow-up path grounded strictly in retrieved links."
    )
