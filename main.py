...
if not _CORE_PATH.exists():
    raise RuntimeError("USE v464 package integrity failure: use_core.py is missing.")
_core_bytes = _CORE_PATH.read_bytes()
_core_runtime_sha = hashlib.sha1(f"blob {len(_core_bytes)}\0".encode() + _core_bytes).hexdigest()
if _core_runtime_sha != EXPECTED_CORE_BLOB_SHA:
    raise RuntimeError(f"USE v464 package integrity failure: expected protected core blob sha={EXPECTED_CORE_BLOB_SHA}, actual={_core_runtime_sha}")
...
if _original_handle_query is None:
    raise RuntimeError("USE v464 package integrity failure: API query handler is unavailable.")
if not callable(_original_evidence_sufficiency_unavailable_response):
    raise RuntimeError("USE v464 package integrity failure: evidence-gap response boundary is unavailable.")
...
def _definition_candidates(query: str, primary: dict):
...
def _concept_synthesis(query: str, primary: dict):
    ...
    if len(chosen) >= 2:
        return f"Taken together, the retrieved material presents {subject} as " + re.sub(r"^[Tt]his collection explores|^[Tt]hese writings consider", "a concept explored through", " ".join(chosen), count=1)
    return chosen[0]
...
def _v464_evidence_gap_boundary(user_query: str, canonical_link_context: str = "") -> str:
...
_V464_BOUNDARY_QUERY="What is the Living Archive?"
_V464_BOUNDARY_AUDIT=_v464_evidence_gap_boundary(_V464_BOUNDARY_QUERY,"")
if "living archive" not in _V464_BOUNDARY_AUDIT.casefold() or "USE" in _V464_BOUNDARY_AUDIT:
    raise RuntimeError("USE v464 visitor boundary audit failed.")
...
use_core.generate_llm_response=_v464_generate_boundary
use_core._evidence_sufficiency_unavailable_response=_v464_evidence_gap_boundary