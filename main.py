...
def _build_generation_messages(user_query: str, intent: str, retrieved_context_blocks: str, response_contract: str):
    result = _call_original(_original_build_generation_messages, user_query, intent, retrieved_context_blocks, response_contract)
    if result is None:
        return use_core._build_generation_messages if callable(getattr(use_core, "_build_generation_messages", None)) else []
    return result


def _clean_generation_output(value: str) -> str:
    result = _call_original(_original_clean_generation_output, value)
    return value if result is None else result


def _run_generation_attempt(*args, **kwargs):
    return _call_original(_original_run_generation_attempt, *args, **kwargs)


def _run_provider_completion_recovery(*args, **kwargs):
    return _call_original(_original_run_provider_completion_recovery, *args, **kwargs)
...
def _v337_apply_recommendation_authority(value: str, user_query: str, retrieved_context_blocks: str) -> str:
    fn = _original_recommendation_output_authority
    if callable(fn):
        try:
...
def _v338_final_answer_boundary(value: str, user_query: str, retrieved_context_blocks: str, canonical_link_context: str) -> str:
    ...
    identity_fn = _original_recommendation_resource_identity
