...
def _select_secondary_pathways(docs: list, primary_title: str, profile: dict, limit: int = 2) -> list:
    ...
    for doc in docs:
        ...
        if profile.get("sensitive") and not profile.get("explicit_framework") and not profile.get("risk") and _is_acute_risk_resource(doc):
            continue
        score = 0
        if profile.get("grief") and re.search(r"\b(?:grief|loss|death|mortality|meaning|continuity)\b", corpus, re.I):
            score += 3
        if profile.get("meaning") and re.search(r"\b(?:meaning|identity|purpose|perspective|wisdom|continuity)\b", corpus, re.I):
            score += 2
        role = _secondary_role(doc, profile)
        if role in used_roles:
            score -= 4
        if profile.get("sensitive") and not profile.get("explicit_framework") and re.search(r"\b(?:afterlife|reincarnation)\b", corpus, re.I):
            score -= 3
        if score > 0:
...
def _archive_bridge(profile: dict, meta: dict, secondaries: list) -> str:
    ...
    for d in secondaries:
        secondary_corpora.append(f"{_normalize_title(d.get('title') or '')} {re.sub(r'\s+', ' ', str(d.get('text') or '').strip())}")
    ...
    if profile.get("meaning") or re.search(r"\b(?:meaning|purpose|perspective|wisdom)\b", corpus):
    ...
    if not axes:
        return ""
    if len(axes) == 1:
        return f"Taken together, the nearby material gives this question a wider frame around {axes[0]}."
    if len(axes) == 2:
        return f"Taken together, the nearby material gives this question a wider frame around {axes[0]} and {axes[1]}."
...
def _build_sensitive_recommendation_answer(user_query: str, primary: dict, docs: list) -> str:
    ...
    if architecture["archive_bridge"]:
    ...
    if architecture["secondaries"]:
        ...
        for doc in architecture["secondaries"]:
            ...
            if link:
                links.append(f"{link} — a way to explore {_secondary_role(doc, profile)}.")
        if links:
...
def _is_query_aligned_transition_doorway(doc: dict, query: str) -> bool:
    ...
    if not profile["transition"] or profile["explicit_framework"]:
        return False
...
def _transition_retrieval_strategy(query: str):
    ...
    for doc in list(retrieved or []):
        ...
        if not key or key in seen or not _is_query_aligned_transition_doorway(doc, query):
            continue
        seen.add(key)
...
def _direct_open_transition_response(query: str, docs: list) -> dict:
    ...
    url = str(primary.get("url") or primary.get("canonical_url") or "").strip()
    if not title or not re.match(r"^https?://\S+$", url, re.I):
        return {"response": "A possible place to begin is with the transition itself: what changed, what feels uncertain now, and what remains open.", "resources": []}
