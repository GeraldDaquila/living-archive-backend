...
def _v340_orientation_boundary(user_query: str, answer: str, retrieved_context_blocks: str) -> str:
    ...
    # v340 root contract: grounded topical inquiries are Guide requests even when the provider returned ordinary prose.
    # The existing v339 recommendation path remains separate; topical orientation is deterministic here.
...
def _v339_finalize_generation_response(*args, **kwargs):
    ...
    if not use_core._is_recommendation_question(user_query):
        ...
        if oriented != str(value or "").strip():
            print("USE v340 universal orientation boundary: grounded Guide doorway used for topical inquiry."); return oriented
        return value
