# USE PRODUCTION VERSION: v289 — Embedding Runtime Thread Optimization: Preserve v288 Provider + Model + Retrieval Behavior + The Guide
...
# deterministic.
DEPLOYMENT_FINGERPRINT = "USE-v289-embedding-runtime-threads-4"

# === CANONICAL BUILD IDENTITY (excluded from payload hash) ===
CANONICAL_BUILD_ID = "USE-BUILD-v289-embedding-runtime-threads-4"
CANONICAL_BUILD_PAYLOAD_SHA256 = "c82649de4b38e1bd9f986a318deaa1bebecfacc9fbf63178c359b3fac4c48788"
...
# v289 retains provider/runtime observability from v288. This reads the
...
try:
    ...
    print(
        "USE v289 embedding-runtime diagnostic: "
...
def generate_embedding(text: str, diagnostic_label: Optional[str] = None) -> List[float]:
    # v289 diagnostic is opt-in so non-diagnostic callers retain the same
    ...
    try:
        ...
        if not embeddings:
            ...
            if diagnostic_label:
                print(
                    "USE v289 embedding-provider diagnostic: "
...
def _function_targeted_candidate_search(question: str) -> List[Dict[str, Any]]:
    ...
    for function_name in ordered_targets[:target_limit]:
        ...
        print(
            "USE v289 function-targeted timing: "
...
def fetch_canonical_context(
    ...
    # v289 inherited fetch-stage diagnostic: preserve the previously decomposed
    ...
    _v289_postretrieval_started = time.perf_counter()
    ...
    print(
        "USE v289 fetch diagnostic: "
        f"canonical_evidence_stage={time.perf_counter() - _v289_postretrieval_started:.3f}s"
    )
    _v289_stage_started = time.perf_counter()
    ...
    print(
        "USE v289 fetch diagnostic: "
        f"authority_and_doorway_stage={time.perf_counter() - _v289_stage_started:.3f}s"
    )
    _v289_stage_started = time.perf_counter()
    ...
    print(
        "USE v289 fetch diagnostic: "
        f"post_selection_gates_stage={time.perf_counter() - _v289_stage_started:.3f}s"
    )
    _v289_stage_started = time.perf_counter()
    ...
    print(
        "USE v289 fetch diagnostic: "
        f"evidence_preservation_stage={time.perf_counter() - _v289_stage_started:.3f}s"
    )
    _v289_stage_started = time.perf_counter()
    ...
    print(
        "USE v289 fetch diagnostic: "
        f"generation_evidence_selection_stage={time.perf_counter() - _v289_stage_started:.3f}s"
    )
    _v289_stage_started = time.perf_counter()
    ...
    print(
        "USE v289 fetch diagnostic: "
        f"context_formatting_stage={time.perf_counter() - _v289_stage_started:.3f}s, "
        f"postretrieval_total={time.perf_counter() - _v289_postretrieval_started:.3f}s"
    )
    print(
        "USE v289 latency: "
