...
def _v335_compact_response_contract(user_query: str, intent: str) -> str:
    ...
    lines = ["[VISITOR RESPONSE CONTRACT — APPLY LAST]", f"mode={mode}; intent={intent};", "Answer first. Answer the visitor’s orientation need rather than merely echoing retrieval.", "Preserve the visitor's terms and agency."]
...
def _secondary_path_context(doc: dict, profile: dict) -> str:
    text = re.sub(r"\s+", " ", str(doc.get("text") or "").strip()); title = _normalize_title(doc.get("title") or ""); corpus = f"{title} {text}"
    if re.search(r"\b(?:afterlife|reincarnation)\b", corpus, re.IGNORECASE): return "for a broader exploration of what different traditions and experiences have made of life after death"
    if re.search(r"\b(?:continuity|connection|bond|relationship|identity)\b", corpus, re.IGNORECASE): return "for the question of whether love, identity, or connection can carry forward in some form"
    if re.search(r"\b(?:grief|loss|mourning|bereavement|mortality|death)\b", corpus, re.IGNORECASE): return "for staying with the lived experience of loss and mortality"
    if re.search(r"\b(?:meaning|purpose|perspective|wisdom)\b", corpus, re.IGNORECASE): return "for making room for meaning, perspective, and the questions that follow difficult experience"
    if re.search(r"\b(?:scientific|psychological|research|clinical|neuroscientific)\b", corpus, re.IGNORECASE): return "for a more grounded or research-oriented way of looking at the question"
    if re.search(r"\b(?:spiritual|religious|mystical|sacred|transcenden)\b", corpus, re.IGNORECASE): return "for exploring spiritual or contemplative possibilities without treating them as settled fact"
    if profile.get("risk") and re.search(r"\b(?:support|safety|crisis|help|care)\b", corpus, re.IGNORECASE): return "for practical support, safety, and care alongside reflection"
...
def _extract_archive_metadata(doc: dict, docs: list) -> dict:
    ...
    def _first(patterns):
        ...
        return ""
    collection=_first(collection_patterns); section=_first(section_patterns)
    related_resources=[]
...
def _archive_context(primary: dict, docs: list) -> str:
    ...
    if meta.get("section"):
        raw_section=str(meta["section"] or "").strip(); clean=re.sub(r"\s+"," ",raw_section).strip(" ,;:—–-.")
    ...
    resources=meta.get("related_resources") or []
    links=[_resource_link(r) for r in resources]
    links=[x for x in links if x]
    if links:
        parts.append("It also belongs to a wider conversation in the Archive, alongside " + ", ".join(links[:-1]) + (", and " + links[-1] if len(links)>1 else links[0]) + ".")
    return " ".join(parts)
...
def _v340_orientation_boundary(user_query: str, answer: str, retrieved_context_blocks: str) -> str:
    ...
    for doc in docs:
        ...
        if title and url and text and re.match(r"^https?://",url,re.IGNORECASE): candidates.append({**doc,"title":title,"url":url,"text":text})
    if not candidates:return value
    low=value.casefold(); fallback_signals=("does not establish a reliable explanation","does not establish a next destination","question remains open","closest places surfaced","relevant material available for your question","could not complete its interpretive response")
    if value and not any(signal in low for signal in fallback_signals):return value
...
def _v339_finalize_generation_response(*args,**kwargs):
    ...
    if not use_core._is_recommendation_question(user_query):
        ...
        if oriented!=str(value or "").strip():
            print("USE v340 universal orientation boundary: grounded Guide doorway used for topical fallback."); return oriented
        return value
