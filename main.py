# USE PRODUCTION VERSION: v365 — transition candidate-set composition
import hashlib
import importlib
import re
...
APP_VERSION = "v365"
DEPLOYMENT_FINGERPRINT = "USE-v365-transition-candidate-composition"
CANONICAL_BUILD_ID = "USE-BUILD-v365-transition-candidate-composition"
...
if not _CORE_PATH.exists(): raise RuntimeError("USE v365 package integrity failure: use_core.py is missing.")
_core_runtime_sha = _git_blob_sha256(_CORE_PATH.read_bytes())
if _core_runtime_sha != EXPECTED_CORE_BLOB_SHA: raise RuntimeError(f"USE v365 package integrity failure: expected protected core blob sha={EXPECTED_CORE_BLOB_SHA}, actual={_core_runtime_sha}")
...
def _whole_question_fit(query,doc):
    profile=_query_profile(query,[]) ; fit,clusters=_transition_evidence_fit(doc,profile)
    axes= sum(int(clusters[k]) for k in ("transition","meaning","experience","grounding"))
    return fit, axes, clusters
...
def _transition_primary_candidate_from_context(query,docs):
    ...
    for i,doc in enumerate(docs):
        fit,axes,clusters=_whole_question_fit(query,doc)
        ...
        if clusters["worldview"] and not explicit_worldview: continue
        authority = fit + (6 if axes==4 and not clusters["worldview"] else 0) + (2 if clusters["open"] and clusters["meaning"] and clusters["experience"] else 0)
        eligible.append(((authority,axes,-int(clusters["worldview"]),-i),doc))
...
def _transition_retrieval_strategy(user_query):
    ...
    if callable(retriever):
        ...
        except Exception as exc: print(f"USE v365 transition strategy: function-targeted retrieval error: {type(exc).__name__}: {exc}")
    ...
    if callable(embed) and callable(query_index):
        ...
        for variant in variants:
            ...
            except Exception as exc: print(f"USE v365 transition strategy: semantic recovery error: {type(exc).__name__}: {exc}")
    profile=_query_profile(query,[]); candidates=[]
    for doc in (recovered if isinstance(recovered,list) else []): candidates.append((1.0,doc))
    candidates += semantic
    scored=[]
    for retrieval_score,doc in candidates:
        fit,axes,clusters=_whole_question_fit(query,doc)
        if not (clusters["transition"] and clusters["meaning"] and clusters["experience"] and clusters["grounding"] and clusters["open"] and fit>=7): continue
        if clusters["worldview"] and not profile.get("explicit_worldview"): continue
        authority=fit+(6 if axes==4 and not clusters["worldview"] else 0)+(2 if clusters["open"] and clusters["meaning"] and clusters["experience"] else 0)
        scored.append((authority,axes,retrieval_score,doc))
    scored.sort(key=lambda x:(x[0],x[1],x[2]),reverse=True)
    return _merge_recovered_documents([], [x[3] for x in scored[:12]])
...
def _guide_answer(user_query,primary,docs):
    ...
    if profile.get("transition"):
        fit_docs=[]; seen=set()
        for d in docs:
            fit,axes,clusters=_whole_question_fit(user_query,d)
...
def _v365_finalize(*args,**kwargs):
    ...
    if not recommendation and (not intent or intent=="TOPICAL_INQUIRY"):
        ...
        if profile.get("transition") and profile.get("open_question"):
            ...
            if selected: print(f"USE v365 TRANSITION EVIDENCE GATE: primary='{_normalize_title(selected.get('title') or '')}', provider_generation_skipped=True"); return _guide_answer(query,selected,merged)
            print("USE v365 TRANSITION EVIDENCE GATE: no sufficiently aligned canonical evidence; provider_generation_skipped=True"); return "The Living Archive does not currently have sufficiently grounded canonical material for this particular question, and I do not want to point you to a resource merely because its wording happens to overlap. It is better to leave the doorway open than pretend an unrelated resource is the right place to begin."
...
use_core.APP_VERSION=APP_VERSION; use_core.DEPLOYMENT_FINGERPRINT=DEPLOYMENT_FINGERPRINT; use_core.CANONICAL_BUILD_ID=CANONICAL_BUILD_ID; use_core.generate_llm_response=_v365_finalize
print(f"USE v365 GUIDE BUILD IDENTITY: build_id={CANONICAL_BUILD_ID}, version={APP_VERSION}, fingerprint={DEPLOYMENT_FINGERPRINT}, source_sha256={RUNTIME_SOURCE_SHA256}, core_blob_sha256={_core_runtime_sha}")